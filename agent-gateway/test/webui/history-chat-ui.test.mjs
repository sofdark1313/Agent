  ...
  const entries = chatUi.parseHistoryMessagesJson(JSON.stringify([
    {
      ...
      agentDisplayContent: "please inspect notes",
      agentAttachments: [
        {
      ...
      content: [
        {
          ...
          name: "Bash",
          input: {
            command: "pnpm -C agent-gateway/web build",
            cwd: "agent-gateway/web",
            root: "workspace",
          },
        },
      ],
...
test("WebUI transcript strips leaked DSML tool call markup from text and thinking", () => {
  const dsml = [
    ...
    '<||DSML|| parameter name="query">Agent DSML markup</||DSML|| parameter>',
    "</||DSML|| invoke>",
    "</||DSML|| tool_calls>",
  ].join("\n");
  const entries = chatUi.parseHistoryMessagesJson(JSON.stringify([
...
test("WebUI transcript hides provider-native web_search tool traces when hosted search exists", () => {
  const webSearchCall = {
    ...
    name: "web_search",
    arguments: { query: "Agent DeepSeek webui search" },
  };
  const entries = chatUi.parseHistoryMessagesJson(JSON.stringify([
    { role: "user", content: "search" },
    {
      ...
      content: [
        { type: "text", text: "searching" },
        {
          ...
          status: "completed",
          queries: ["Agent DeepSeek webui search"],
...
test("WebUI live transcript removes provider-native web_search when hosted search arrives later", () => {
  ...
  turn = applyEventToTurn(turn, {
    ...
    name: "web_search",
    arguments: { query: "Agent DeepSeek live search" },
    round: 1,
  });
  ...
  turn = applyEventToTurn(turn, {
    ...
    provider: "claude_code",
    status: "completed",
    queries: ["Agent DeepSeek live search"],
  ...
  const turn = reduceTurnEvents([
    {
      ...
      name: "WebSearch",
      arguments: { query: "Agent recovered search" },
      round: 1,
    },
    {
      type: "tool_result",
  ...
  const turn = reduceTurnEvents([
    {
      ...
      arguments: { query: "Agent DSML hidden search" },
      round: 1,
    },
  ]);
...
test("turn reducer appends streaming text, dedupes tool cards, and dedupes compaction checkpoints", () => {
  ...
  const checkpoint = {
    ...
    checkpoint: {
      ...
      generatedBy: { providerId: "agent", model: "summary" },
    },
  };
