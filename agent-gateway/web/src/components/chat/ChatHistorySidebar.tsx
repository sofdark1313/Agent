...
const AgentAppMenu = (props: any) => null;
import {
  Blend,
  Cable,
  ChevronRight,
...
export const ChatHistorySidebar = memo(function ChatHistorySidebar(props: ChatHistorySidebarProps) {
  ...
  const getHistoryItemKey = useCallback(
    (index: number) => {
      ...
      return item?.id ? `${item.id}-${index}` : `row-${index}`;
    },
    [items],
  );
        ...
        <div className="shrink-0 border-t border-border/50 bg-[hsl(var(--sidebar-bg))] px-2 py-2">
          {accountAction ? (
            ...
            >
              ...
              <span className="truncate">{t("sidebar.openSettings")}</span>
            </Button>
          )}
        </div>
      </div>
