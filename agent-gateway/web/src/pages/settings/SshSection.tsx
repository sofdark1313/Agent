          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
              ...
              >
                ...
                <Check
                  aria-hidden="true"
                  className={`ml-auto h-4 w-4 shrink-0 text-foreground transition-all duration-200 ${
...
function SshHostCard(props: {
  ...
  if (viewMode === "grid") {
    return (
      <div className="group relative z-0 flex flex-col rounded-2xl border border-border/60 bg-card p-5 transition-all duration-200 hover:z-10 hover:border-border hover:shadow-xs">
        <div className="absolute right-3 top-3">{actions}</div>
        <div className="flex items-start gap-3 pr-12">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-muted text-foreground transition-transform duration-200 group-hover:scale-105">
  ...
  return (
    <div className="group relative z-0 rounded-2xl border border-border/60 bg-card transition-all duration-200 hover:z-10 hover:border-border hover:shadow-xs">
      <div className="settings-card-row flex items-center gap-3 px-4 py-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-muted text-foreground transition-transform duration-200 group-hover:scale-105">
...
function SshViewModeToggle(props: { value: SshViewMode; onChange: (value: SshViewMode) => void }) {
  ...
  return (
    <fieldset className="relative isolate grid min-w-0 grid-cols-2 rounded-lg border border-border/60 bg-muted/30 p-0.5 shadow-inner shadow-black/5">
      ...
      <span
        aria-hidden="true"
        className={`pointer-events-none absolute bottom-0.5 left-0.5 top-0.5 w-[calc(50%-0.125rem)] rounded-md bg-muted shadow-sm shadow-emerald-500/10 ring-1 ring-emerald-500/30 transition-transform duration-200 ease-out motion-reduce:transition-none ${
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border/70 bg-gradient-to-br from-card via-card to-muted/20 p-6 shadow-xs">
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-start gap-3.5">
              ...
              <div>
                <h3 className="text-base font-semibold text-foreground">{t("settings.sshTitle")}</h3>
                <p className="mt-0.5 text-xs text-muted-foreground">{t("settings.sshDesc")}</p>
              </div>
            </div>

            <div className="flex flex-wrap items-center gap-2">
              {hosts.length > 0 ? (
                <div className="flex items-center gap-2 rounded-xl border border-border/50 bg-background/80 px-3 py-1.5 text-xs font-medium text-muted-foreground shadow-2xs">
                  <span className="tabular-nums font-bold text-foreground">{hosts.length}</span>
                  {t("settings.sshCount")}
                </div>
              ) : null}
              ...
              <Button
                variant="outline"
                size="sm"
                className="gap-1.5 rounded-xl border-border/60 bg-background/80 shadow-2xs hover:bg-muted/40"
                onClick={() => setImportOpen(true)}
              >
                <Upload className="h-3.5 w-3.5 text-muted-foreground" />
                {t("settings.sshImport")}
              </Button>
              <Button size="sm" className="gap-1.5 rounded-xl shadow-xs" onClick={openAdd}>
