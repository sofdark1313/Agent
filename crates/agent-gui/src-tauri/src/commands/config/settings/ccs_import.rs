...
pub async fn settings_list_ccswitch_providers() -> Result<CcsProvidersResponse, String> {
    tauri::async_runtime::spawn_blocking(|| {
        ...
        let providers = match path {
            Some(path) => list_ccswitch_agent_providers_from_db(path)?,
            None => Vec::new(),
        };
        let message = if providers.is_empty() {
            let checked = candidates
                ...
                .collect::<Vec<_>>()
                .join("；");
            format!("未发现 ccswitch Agent 供应商，已检查：{checked}")
        } else {
            format!("发现 {} 个 ccswitch Agent 供应商", providers.len())
        };
...
fn list_ccswitch_agent_providers_from_db(
    path: &std::path::Path,
