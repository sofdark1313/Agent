...
Use this reference when writing or reviewing an Agent-compatible skill.
...
- Agent discovers runtime skills from the fixed user Skills root managed by `SkillsManager`.
- `SkillsManager(action=create)` writes runtime skills under that fixed Skills root.
- The repository source copy for built-in project skills lives under `agent-gui/src-tauri/prompt/skills/<skill-name>`, but that is not the runtime installation root.
- The runtime scanner accepts `SKILL.md`, `skill.md`, `skill.json`, and fallback `README.md`.
- Markdown skill entries should begin with YAML frontmatter containing `name` and `description`. `README.md` without frontmatter is supported only as an import/discovery fallback; Agent derives the name from the directory and loads the full README when enabled.
...
---
name: my-skill
description: Do a specific Agent workflow. Use when the user asks for the exact tasks this skill supports.
---
```
...
`allowed-tools` is accepted only as compatibility metadata when importing existing skills. Agent currently enforces tool access through the chat Skills selector and runtime `SkillAccessPolicy`, not through per-skill frontmatter.
...
- Use `SkillsManager` actions for Agent skill creation, installation, listing, validation, and packaging. Do not bundle Python helper scripts for these core workflows.
