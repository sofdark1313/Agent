...
test("application modules live at the repository root", () => {
  assert.equal(fs.existsSync(path.join(repositoryRoot, "agent-gui", "package.json")), true);
  assert.equal(fs.existsSync(path.join(repositoryRoot, "agent-gateway", "go.mod")), true);
  assert.equal(fs.existsSync(path.join(repositoryRoot, "crates")), false);
  ...
  for (const relativePath of [
    ...
    assert.doesNotMatch(content, /crates[\\/]agent-(?:gui|gateway)/, relativePath);
  }
});
