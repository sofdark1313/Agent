...
test("AI release notes script falls back when no API key is configured", () => {
  const dir = mkdtempSync(path.join(tmpdir(), "agent-notes-"));
  try {
...
test("AI release notes script calls Chat Completions API and writes markdown", async () => {
  ...
  try {
    ...
    assert.match(readFileSync(outputPath, "utf8"), /^# Agent v0\.1\.6/);
  ...
  const server = http.createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      response.end(
        JSON.stringify({
          output_text:
            "# Agent v0.1.6\n\n> Release notes generated by the Responses fallback.\n\n## Overview\n\nAgent still publishes AI release notes when chat output is empty.",
        }),
      );
    });
  });
